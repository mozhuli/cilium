...
type LocalObserverServer struct {
	...
	startTime      time.Time
	onGetFlowss    []OnGetFlows
}

// A LocalObserverServerOption sets an option on a LocalServer.
type LocalObserverServerOption func(*LocalObserverServer)

// An OnGetFlows is a GetFlows callback.
...
// WithOnGetFlows adds onGetFlows as a callback when GetFlows is called.
func WithOnGetFlows(onGetFlows OnGetFlows) LocalObserverServerOption {
	return func(s *LocalObserverServer) {
		s.onGetFlowss = append(s.onGetFlowss, onGetFlows)
	}
}
...
// NewLocalServer returns a new LocalServer with the given options.
func NewLocalServer(options ...LocalObserverServerOption) *LocalObserverServer {
